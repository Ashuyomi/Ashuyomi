...
package eu.kanade.tachiyomi.util.chapter.exh
...
object EXHMigrations {
        ...
        try {
            if (oldVersion < BuildConfig.VERSION_CODE) {
                ...
                if (oldVersion under 4) {
                    eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.updateSourceId(
                        HBROWSE_SOURCE_ID,
                        6912
                    )
                    // Migrate BHrowse URLs
                    val hBrowseManga = runBlocking { eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.getMangaBySource.await(HBROWSE_SOURCE_ID) }
                    val mangaUpdates = hBrowseManga.map {
                        MangaUpdate(it.id, url = it.url + "/c00001/")
                    }

                    runBlocking {
                        eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.updateManga.awaitAll(mangaUpdates)
                    }
                }
                ...
                if (oldVersion under 7) {
                    val mergedMangas = runBlocking { eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.getMangaBySource.await(MERGED_SOURCE_ID) }

                    if (mergedMangas.isNotEmpty()) {
                        val mangaConfigs = mergedMangas.mapNotNull { mergedManga -> eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.readMangaConfig(
                            mergedManga
                        )?.let { mergedManga to it } }
                        if (mangaConfigs.isNotEmpty()) {
                            ...
                            mangaConfigs.onEach { mergedManga ->
                                val newFirst = mergedManga.second.children.firstOrNull()?.url?.let {
                                    if (runBlocking { eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.getManga.await(it, MERGED_SOURCE_ID) } != null) return@onEach
                            ...
                            runBlocking {
                                eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.updateManga.awaitAll(mangaToUpdate)
                                eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.insertMergedReference.awaitAll(mergedMangaReferences)
                            }

                            val loadedMangaList = mangaConfigs.map { it.second.children }.flatten().mapNotNull { it.load() }.distinct()
                            val chapters = runBlocking { eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.handler.awaitList { ehQueries.getChaptersByMangaIds(mergedMangas.map { it.id }, chapterMapper) } }
                            val mergedMangaChapters = runBlocking { eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.handler.awaitList { ehQueries.getChaptersByMangaIds(loadedMangaList.map { it.manga.id }, chapterMapper) } }
                            ...
                            val parsedChapters = chapters.filter { it.read || it.lastPageRead != 0L }.mapNotNull { chapter -> eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.readUrlConfig(
                                chapter.url
                            )?.let { chapter to it } }
                            ...
                            runBlocking {
                                eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.deleteChapters.await(mergedMangaChapters.map { it.id })
                                eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.updateChapter.awaitAll(chaptersToUpdate)
                            }
                        }
                    }
                }
                ...
                if (oldVersion under 17) {
                    ...
                    runBlocking {
                        eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.handler.await { ehQueries.deleteBySyncId(6) }
                    }
                }
                ...
                if (oldVersion under 31) {
                    runBlocking {
                        ...
                        if (!savedSearch.isNullOrEmpty()) {
                            eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.insertSavedSearch.awaitAll(savedSearch)
                        }
                        ...
                        if (!feedSavedSearch.isNullOrEmpty()) {
                            eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.insertFeedSavedSearch.awaitAll(feedSavedSearch)
                        }
                    }
                    prefs.edit(commit = true) {
                ...
                if (oldVersion under 38) {
                    ...
                    runBlocking {
                        eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.handler.await(true) {
    ...
    fun migrateBackupEntry(manga: Manga) {
        if (manga.source == 6907L) {
            ...
            manga.url =
                eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.getUrlWithoutDomain(manga.url)
        }
    ...
    private data class MangaConfig(
        @SerialName("c")
        val children: List<eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.MangaSource>,
    ) {
        companion object {
            fun readFromUrl(url: String): eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.MangaConfig? {
                return try {
    ...
    private fun readMangaConfig(manga: DomainManga): eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.MangaConfig? {
        return eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.MangaConfig.Companion.readFromUrl(
            manga.url
        )
    }
    ...
    private data class MangaSource(
        ...
        fun load(): eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.LoadedMangaSource? {
            val manga = runBlocking { eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.getManga.await(url, source) } ?: return null
            val source = eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.sourceManager.getOrStub(source)
            return eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.LoadedMangaSource(
                source,
                manga
            )
        }
    ...
    private fun readUrlConfig(url: String): eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.UrlConfig? {
        return try {
            Json.decodeFromString(url)
        } catch (e: Exception) {
            null
    ...
    private fun updateSourceId(newId: Long, oldId: Long) {
        runBlocking {
            eu.kanade.tachiyomi.util.chapter.exh.EXHMigrations.handler.await { ehQueries.migrateSource(newId, oldId) }
        }
    }
}
